...
import { careersListAtom, checkedListAtom, totalPageAtom } from "@/atom";
import { useEffect, useState } from "react";
import { useSearchParams, usePathname } from "next/navigation";
import Pagination from "../Pagination/Pagination";
import CustomModal from "../Modal/Confirm";
import { deleteCareers, getCareersList } from "@/hooks/useEvents";
...
const CareersList = () => {
  ...
  const userDelete = async () => {
    checkedElements.forEach(async (element) => {
      await deleteCareers(Number(element));
    });
    getData();
    setChechedElements([]);
    setIsOpen(false);
  ...
  const getData = async () => {
    ...
    const response = await getCareersList(
      String(userToken),
      Number(page),
      Number(size)
    );
          ...
          {isOpen ? (
            <CustomModal>
              <h2 className="text-xl text-black">
                ({checkedElements?.length}) Careers will <br /> be deleted
              </h2>
